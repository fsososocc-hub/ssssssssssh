...
import { 
  ...
  ChevronDown, ChevronRight, Plus, Check, PanelLeftClose, PanelLeft, LayoutGrid, CreditCard, Sparkles
} from 'lucide-react';
...
export default function Sidebar({ currentTab, onSelectTab, settings, onOpenSettings }: SidebarProps) {
  ...
  // Auto collapse on POS setup
  useEffect(() => {
    if (currentTab === 'pos-setup') {
      setIsCollapsed(true);
    }
  }, [currentTab]);
  ...
  const [selectedStore, setSelectedStore] = useState<string>('Atelier Noir 官方旗舰店');
  ...
  const handleCreateNewStore = () => {
    const freshName = prompt("请输入自定义分店名号名称：", "米兰时尚廊");
    if (freshName) {
      setMockStores(prev => [...prev, `${freshName} 店`]);
      setSelectedStore(`${freshName} 店`);
    }
    setStoreDropdownOpen(false);
  };
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-1 scrollbar-thin scrollbar-thumb-neutral-800">
            ...
            <div>
              <button
                ...
                className={`flex items-center justify-between w-full px-3 py-2 text-xs rounded-lg transition-all text-neutral-300 hover:bg-neutral-800 hover:text-white cursor-pointer ${
                  currentTab.includes('order') || currentTab === 'drafts' || currentTab === 'abandoned' || currentTab === 'returns' ? 'text-white font-bold' : ''
                }`}
              >
                <div className="flex items-center space-x-3">
              ...
              {openSubMenus.orders && (
                <div className="mt-1 space-y-0.5 border-l border-neutral-800 ml-5.5">
                  ...
                  <button onClick={() => onSelectTab('returns')} className={getSubMenuClass('returns')}>
                    <span>{translate('returns_orders', settings)}</span>
                  </button>
                </div>
              )}
            </div>
        ...
        <button
          onClick={() => onSelectTab('ai-apps')}
          className={getMenuClass('ai-apps')}
        >
          <div className="flex items-center space-x-3">
            ...
            {!isCollapsed && (
              <span className="flex items-center font-medium text-xs">
                智能大脑
                <span className="ml-1.5 px-1 py-0.2 bg-indigo-600 text-white text-[8px] rounded font-mono font-normal uppercase">APP</span>
              </span>
            )}
          </div>
        </button>
